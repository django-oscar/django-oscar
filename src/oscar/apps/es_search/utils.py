...
def terms_buckets_to_values_list(buckets):
    """
    Turn `terms` search result buckets into a list of values where each value appears in the list a number of times
    equivalent to the result's doc_count.
    :param buckets: The results buckets
...
def get_auto_ranges(chunks):
    """
    Try to generate similar size range groups
    :param chunks: Sorted list of lists of positive integers each representing a hit from search results
    :return: List of ranges in dict format e.g [{'from': 0: 'to': 10, 'doc_count': 4}]
    """
...
def agg_range_to_filters(agg, values):
    """
    Convert a list of values to list of range filters.

    Args:
        agg: Range facet from which the field name is derived
        values: A list of `from-to` values used to generate range filters

    Returns: A list of Range filters, one filter for each supplied value
...
def agg_terms_to_filters(agg, values):
    """
    Convert a list of values to a list of Term filters.

    Args:
        agg: TermFacet from which the field name is derived
        values: A list of values to be used for filtering

    Returns: List of Term filters, one filter for each supplied value.
...
def agg_histogram_to_filters(agg, values):
    """
    Convert a list of histogram values to a list of Range filters

    Args:
        agg: HistogramFacet from which the field name and interval are derived
        values: List of values that will be used to create ranges based on the facet's interval

    Returns: List of Range filters
...
def agg_to_filter(agg, value):
    """
    Convert the given values to a search filter. The type of filter is determined from the type of agg
    Args:
        agg: A Facet type used to decide the type of search filter created
        value: A list of values used to create the filters

    Returns: A boolean filter generated by OR-ing all the filters created from the given values.
