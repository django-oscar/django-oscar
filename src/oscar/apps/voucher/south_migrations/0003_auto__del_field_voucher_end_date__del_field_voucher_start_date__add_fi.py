...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.delete_column('voucher_voucher', 'start_date')
        db.delete_column('voucher_voucher', 'end_date')

    def backwards(self, orm):

        db.add_column('voucher_voucher', 'start_date',
            self.gf('django.db.models.fields.DateField')(null=True))

        db.add_column('voucher_voucher', 'end_date',
            self.gf('django.db.models.fields.DateField')(null=True))

        Voucher = orm['voucher.Voucher']
        for voucher in Voucher.objects.all():
            start_datetime = voucher.start_datetime
            voucher.start_date= datetime.date(
                start_datetime.year, start_datetime.month, start_datetime.day)
            end_datetime = voucher.end_datetime
            voucher.end_date = datetime.date(
                end_datetime.year, end_datetime.month, end_datetime.day)
            voucher.save()

        # No need for column to be nullable now.
        db.alter_column(u'voucher_voucher', 'end_date', self.gf(
            'django.db.models.fields.DateField')(null=False))
        db.alter_column(u'voucher_voucher', 'start_date', self.gf(
            'django.db.models.fields.DateField')(null=False))    
