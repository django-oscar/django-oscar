...
enforced. Each view class now has a ``pre_conditions`` attribute which is a
list of method names. Each method is run within the ``dispatch`` method of the
...
Further, the ``PaymentDetailsView`` has been re-organised to make extension easier. For
instance, several new methods have been introduced to allow fine-grained
overriding of functionality:
