...
default as weight-based shipping methods are not enabled by default. To add it to
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
If Oscar is running with a Solr-powered search backend, the category browsing now offers faceting
(e.g. filter by price range, or product type).
This is implemented via a new ``SearchHandler`` interface, which will eventually replace the
tight coupling between Haystack and Oscar. It therefore paves the way for better support for
