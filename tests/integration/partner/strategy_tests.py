...
class TestDefaultStrategy(TestCase):
    ...
    def test_one_stockrecord(self):
        product = factories.StandaloneProductFactory(stockrecords__price_excl_tax=D('1.99'), 
                                                     stockrecords__num_in_stock=4)
        info = self.strategy.fetch_for_product(product)
    ...
    def test_product_which_doesnt_track_stock(self):
        product = factories.StandaloneProductFactory(product_class__name="Digital",
                                                     ...
                                                     stockrecords__price_excl_tax=D('1.99'), 
    ...
    def test_free_product_is_available_to_buy(self):
        product = factories.StandaloneProductFactory(stockrecords__price_excl_tax=D('0'), 
                                                     stockrecords__num_in_stock=1)
        info = self.strategy.fetch_for_product(product)
        self.assertTrue(info.availability.is_available_to_buy)
...
class TestDefaultStrategyForParentProductWhoseVariantsHaveNoStockRecords(TestCase):
    ...
    def setUp(self):
        self.strategy = strategy.Default()
        parent = factories.ParentProductFactory(children=[])
        for __ in range(3):
            factories.ChildProductFactory(parent=parent, stockrecords=[])
...
class TestDefaultStrategyForParentProductWithInStockVariant(TestCase):
    ...
    def setUp(self):
        self.strategy = strategy.Default()
        parent = factories.ParentProductFactory(children__stockrecords__price_excl_tax=D('10.00'),
                                                children__stockrecords__num_in_stock=3)
        for __ in range(2):
            factories.ChildProductFactory(parent=parent, stockrecords=[])
        self.info = self.strategy.fetch_for_parent(parent)
...
class TestDefaultStrategyForParentProductWithOutOfStockVariant(TestCase):
    ...
    def setUp(self):
        self.strategy = strategy.Default()
        parent = factories.ParentProductFactory(children__stockrecords__price_excl_tax=D('10.00'),
                                                children__stockrecords__num_in_stock=0)
        for __ in range(2):
            factories.ChildProductFactory(parent=parent, stockrecords=[])
        self.info = self.strategy.fetch_for_parent(parent)
