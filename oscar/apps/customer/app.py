...
class CustomerApplication(Application):
    ...
    def get_urls(self):
        urlpatterns = patterns('',
            ...
            url(r'wishlists/add/with_product/(?P<product_pk>\d+)/$',
                login_required(self.wishlists_create_view.as_view()),
                name='wishlists-create'),
            ...
            url(r'wishlists/(?P<key>[a-z0-9]+)/add_product/(?P<product_pk>\d+)/',
                login_required(self.wishlists_add_product_view.as_view()),
                name='wishlists-add-product'),
            url(r'wishlists/add_product/(?P<product_pk>\d+)/',
                login_required(self.wishlists_add_product_view.as_view()),
                name='wishlists-add-product'),
            url(r'wishlists/(?P<key>[a-z0-9]+)/delete_line/(?P<line_pk>\d+)/',
                login_required(self.wishlists_remove_product.as_view()),
                name='wishlists-remove-product'),
            url(r'wishlists/(?P<key>[a-z0-9]+)/delete_product/(?P<product_pk>\d+)/',
                login_required(self.wishlists_remove_product.as_view()),
                name='wishlists-remove-product'),
            url(r'wishlists/(?P<key>[a-z0-9]+)/move/(?P<line_pk>\d+)/to/(?P<to_key>[a-z0-9]+)/$',
