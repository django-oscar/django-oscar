...
class AccountSummaryView(ListView):
    ...
    def get_filtered_queryset(self, date_from, date_to):
        return self.model._default_manager.filter(user=self.request.user, date_placed__range=[date_from, date_to])

    def get_context_data(self, *args, **kwargs):
        ctx = super(AccountSummaryView, self).get_context_data(*args, **kwargs)
        if self.request.REQUEST.has_key('date_from') or self.request.REQUEST.has_key('date_to'):
            filter_form = SearchByDateRangeForm(self.request.REQUEST)
            ctx['search_date_form'] = filter_form
            # If the form is valid perform filtering on user orders
            if filter_form.is_valid():
                ctx['orders'] = self.get_filtered_queryset(self.request.REQUEST['date_from'], self.request.REQUEST['date_to'])
        else:
            ctx['search_date_form'] = SearchByDateRangeForm()
        return ctx
