...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        orm.UserAddress.objects.filter(first_name__isnull=True).update(first_name='')
        orm.UserAddress.objects.filter(title__isnull=True).update(title='')
        orm.UserAddress.objects.filter(notes__isnull=True).update(notes='')
        orm.UserAddress.objects.filter(line4__isnull=True).update(line4='')
        orm.UserAddress.objects.filter(line3__isnull=True).update(line3='')
        orm.UserAddress.objects.filter(line2__isnull=True).update(line2='')
        orm.UserAddress.objects.filter(state__isnull=True).update(state='')
        orm.UserAddress.objects.filter(postcode__isnull=True).update(postcode='')
        orm.Country.objects.filter(iso_3166_1_a3__isnull=True).update(iso_3166_1_a3='')
